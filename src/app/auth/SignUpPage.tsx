...
import { User, Mail, Lock, Eye, EyeOff, Check, X, ShieldCheck, AlertCircle, Briefcase } from 'lucide-react';
import { Button } from '../../components/ui/Button';
import { Logo } from '../../components/ui/Logo';
import { useAuth } from '../../contexts/AuthContext';
import { cn } from '../../lib/utils';

// All roles grouped by department — mirrors User.js ROLE_GROUPS
const ROLE_OPTIONS = [
  {
    group: 'Creative',
    roles: [
...
export default function SignUpPage() {
  ...
  const isValid = strengthScore >= 5 && passwordsMatch && !!formData.role;
  ...
  const inputClass = "w-full bg-background border border-border rounded-xl py-3 pl-12 pr-4 text-text placeholder:text-text-muted focus:outline-none focus:ring-2 focus:ring-primary/50 transition-all";
  ...
  return (
    <div className="min-h-screen bg-background flex items-center justify-center p-6 relative overflow-hidden">
      ...
      <div className="absolute inset-0 z-0 pointer-events-none">
        {[...Array(16)].map((_, i) => (
          <motion.div key={i}
            initial={{ x: Math.random() * 1200, y: Math.random() * 800, opacity: Math.random() * 0.4 }}
            animate={{ y: [null, Math.random() * -80, null], opacity: [0.2, 0.5, 0.2] }}
      ...
      <motion.div
        ...
        className="w-full max-w-2xl glass p-8 md:p-12 rounded-2xl relative z-10 shadow-2xl"
      >
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-2">
              ...
              <div className="relative group">
                ...
                <input type="email" required value={formData.email}
                  ...
                  placeholder="john@agency.com" className={inputClass} />
              </div>
            </div>
          </div>
          ...
          <div className="space-y-2">
            ...
            <div className="relative group">
              <Briefcase className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-text-muted group-focus-within:text-primary transition-colors pointer-events-none" />
              <select
                required
                value={formData.role}
                onChange={e => setFormData({ ...formData, role: e.target.value })}
                className={cn(inputClass, 'appearance-none cursor-pointer', !formData.role && 'text-text-muted')}
              >
            ...
            {formData.role && (
              <p className="text-[10px] text-text-muted ml-1">
                {ROLE_OPTIONS.find(g => g.roles.some(r => r.value === formData.role))?.group} department
              </p>
            )}
          </div>
            ...
            <div className="space-y-2">
              ...
              <div className="mt-2 space-y-2">
                <div className="flex gap-1 h-1">
                  {[...Array(5)].map((_, i) => (
                    <div key={i} className={cn('flex-1 rounded-full transition-all duration-500',
                      i < strengthScore
                ...
                <div className="grid grid-cols-1 gap-y-1">
                  {passwordRequirements.map((req, i) => (
                    <div key={i} className="flex items-center gap-2">
                      {req.met ? <Check className="w-3 h-3 text-green-500 shrink-0" /> : <div className="w-3 h-3 rounded-full border border-border shrink-0" />}
                      <span className={cn('text-[10px]', req.met ? 'text-green-500/70' : 'text-text-muted')}>{req.label}</span>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div className="space-y-2">
              ...
              <div className="relative group">
                ...
                <input type={showPassword ? 'text' : 'password'} required value={formData.confirmPassword}
                  ...
                  placeholder="••••••••"
                  className={cn('w-full bg-background border rounded-xl py-3 pl-12 pr-4 text-text placeholder:text-text-muted focus:outline-none focus:ring-2 transition-all',
              ...
              {formData.confirmPassword.length > 0 && !passwordsMatch && (
                <p className="text-[10px] text-red-500 mt-1 flex items-center gap-1"><X className="w-3 h-3" /> Passwords do not match</p>
              )}
              {formData.confirmPassword.length > 0 && passwordsMatch && (
                <p className="text-[10px] text-green-500 mt-1 flex items-center gap-1"><Check className="w-3 h-3" /> Passwords match</p>
              )}
            </div>
          </div>
          ...
          <Button type="submit" className="w-full py-4 text-lg" isLoading={isLoading} disabled={!isValid}>
